...
public class FinalBitmap {
	...
	private void doDisplay(View imageView, String uri, BitmapDisplayConfig displayConfig) {
		...
		checkImageSize(imageView, displayConfig);
		L.d(TAG, "load image, w,h=" + displayConfig.getBitmapWidth() + "," + displayConfig.getBitmapHeight());
		Bitmap bitmap = null;

		String cacheKey = uri + "_" + displayConfig.getBitmapWidth() + "x" + displayConfig.getBitmapHeight();
		if (mImageCache != null) {
			bitmap = mImageCache.getBitmapFromMemoryCache(cacheKey);
			if (bitmap != null) L.d(TAG, "load img from memory cache");
		}
	...
	private void checkImageSize(View view, BitmapDisplayConfig displayConfig) {
		//set width
			int w = view.getWidth();
			if (w <= 0) {
			...
			if (w <= 0) {
				w = mConfig.defaultDisplayConfig.getBitmapWidth();
			}
			displayConfig.setBitmapWidth(w);
		...
		//set height
			...
			if (h <= 0) {
				h = mConfig.defaultDisplayConfig.getBitmapHeight();
			}
			...
			displayConfig.setBitmapHeight(h);
	}
	...
	private class BitmapLoadAndDisplayTask extends AsyncTask<Object, Void, Bitmap> {
		...
		protected Bitmap doInBackground(Object... params) {
			...
			if (bitmap != null && mImageCache != null) {
				String cacheKey = dataString + "_" + displayConfig.getBitmapWidth() + "x"
						+ displayConfig.getBitmapHeight();
				mImageCache.addToMemoryCache(cacheKey, bitmap);
			}
