...
public class HttpHandler extends AsyncTask<Object, Object, Object> implements EntityCallBack {
	...
	private void makeRequestWithRetries(HttpUriRequest request) throws IOException {
		if (isResume && targetUrl != null) {
			...
			L.d(TAG, "start file request, url=" + request.getURI() + ", path=" + targetUrl + ", isResume=" + isResume
					+ (fileLen > 0 ? "RANGE : bytes=" + fileLen + "-" : ""));
		}
	...
	protected Object doInBackground(Object... params) {
		...
		try {
			...
			publishProgress(UPDATE_FINISH); // 完成
	...
	protected void onProgressUpdate(Object... values) {
		...
		case UPDATE_LOADING:
			...
			break;
		case UPDATE_FAILURE:
			if (callback != null) callback.onFailure((Exception) values[1], (String) values[3]);
			break;
		case UPDATE_SUCCESS:
			if (callback != null) callback.onSuccess(values[1]);
			break;
		case UPDATE_FINISH:
	...
	private void handleResponse(HttpResponse response) {
		...
		L.d(TAG, "connect result status=" + status);
		if (status.getStatusCode() >= 300) {
			...
			try {
				...
				if (entity != null) {
					...
					if (targetUrl != null) {
						responseBody = mFileEntityHandler.handleEntity(entity,response, this, targetUrl, isResume);
					} else {
		...
		if (callback != null && callback.isProgress()) {
			if (mustNoticeUI) {
				...
				if (thisTime - time >= callback.getRate()) {
					...
					if(targetUrl!=null){
						L.d(TAG, "publich file progress, file=" + new File(targetUrl).getName() + ", progress="
								+ (float) current / count * 100f + "%");
					}
				}
			}
		}
