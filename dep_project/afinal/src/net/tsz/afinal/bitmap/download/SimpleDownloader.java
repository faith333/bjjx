...
public class SimpleDownloader implements Downloader {
	...
	public byte[] download(String urlString) {
		...
		if (urlString.trim().toLowerCase().startsWith("http")) {
			L.d(TAG, "download from http, url=" + urlString);
			return getFromHttp(urlString);
		} else if (urlString.trim().toLowerCase().startsWith("file:")) {
			L.d(TAG, "download from file, url=" + urlString);
			try {
				...
				L.e(TAG, "Error in read from file - " + urlString + " : " + e);
			}
		} else {
	...
	private byte[] getFromFile(File file) {
		...
		try {
			...
			L.e(TAG, "Error in read from file - " + file + " : " + e);
		} finally {
			if (fis != null) {
				try {
					fis.close();
	...
	private byte[] getFromHttp(String urlString) {
		...
		try {
			...
			L.e(TAG, "Error in downloadBitmap - " + urlString + " : " + e);
		} finally {
