...
public class BitmapProcess {
	...
	public Bitmap getBitmap(String url, BitmapDisplayConfig config) {
		String cacheKey = url + "_" + config.getBitmapWidth() + "x" + config.getBitmapHeight();
		Bitmap bitmap = getFromDisk(cacheKey, config);
		...
		if (bitmap == null) {
			...
			if (data != null && data.length > 0) {
				...
				mCache.addToDiskCache(cacheKey, data);
				Log.d(TAG, "load img from network.");
			}
		}else{
			L.d(TAG, "load img from disk cache");
		}
